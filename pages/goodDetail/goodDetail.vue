	...
	export default {
		...
		methods: {
			goShopcar() {
				uni.switchTab({
					...
					success: function() {
						...
						// console.log('switchtab:',getCurrentPages().pop())
					}
				})
			},
			...
			onShareAppMessage(res) {
				return {
					title: '水果商城',
					path: '/pages/goodDetail/goodDetailgood_id=' + goodId
				}
			}
		}
	}
...
<style>
	...
	.addshopcarbox>view:first-child {
		width: 150rpx;
		height: 100%;
		text-align: center;
		line-height: 40px;
	...
	.addshopcarbox>view:first-child .iconfont {
		font-size: 22px;
		color: #888;
	}

	.addshopcarbox>view:last-child {
		flex: 1;
	}

	.addshopcarbox>view:last-child>view {
		width: 90%;
		margin-left: 5%;
		border-radius: 4px;
		text-align: center;
